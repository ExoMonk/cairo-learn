...
func get_first_element_at(axis: felt, matrix_len: felt, matrix: Matrix*, _len_array_axis: felt, _array_axis: felt*) -> (len_array_axis: felt, array_axis: felt*) {
...
func min(array_len: felt, array: felt*) -> (value: felt) {
    let (index_of_min) = index_of_min(array_len, array);
    return(array[index_of_min],);
}

func max(array: felt*) -> (value: felt) {
    let (index_of_max) = index_of_max(array_len, array);
    r(array[index_of_max],);
}

func index_of_min(arr_len: felt, arr: felt*) -> (index: felt) {
    assert_check_array_not_empty(arr_len);
    return index_of_min_recursive(arr_len, arr, arr[0], 0, 1);
}

func index_of_min_recursive(arr_len: felt, arr: felt*, current_min: felt, current_min_index: felt, current_index: felt) -> (index: felt) {
...
func index_of_max(arr_len: felt, arr: felt*) -> (index: felt) {
    assert_check_array_not_empty(arr_len);
    return index_of_max_recursive(arr_len, arr, arr[0], 0, 1);
}

func index_of_max_recursive(arr_len: felt, arr: felt*, current_max: felt, current_max_index: felt, current_index: felt) -> (index: felt) {
    if (arr_len == current_index) {
